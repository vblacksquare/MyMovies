...
class SiteyummyanimeParser(Parser):
    source = Source.siteyummyanime
    login_hash = None
    ...
    async def _search(self, query: str) -> list[Movie]:
        async with self.session.get(
            url=f"https://site.yummyani.me/search",
            params={
                "word": query
            },
            headers={
        ...
        items = soup.select("div.gsc-webResult.gsc-result")

        movies = []
        for item in items:
            link_el = item.select_one("a.gs-title")
